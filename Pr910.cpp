...
void top3(game* games, int size) {
    ...
    for (int i = 0; i < 3 && i < size; i++) {
        cout << temp[i].name << " - " << temp[i].price << " руб" << endl;
        cout << "Студия: " << temp[i].studio << endl;
        cout << "Аудитория: " << temp[i].audience << " млн" << endl;
        cout << "Год выпуска: " << temp[i].info.release << endl;
        cout << "Рейтинг: " << temp[i].info.rating << "/10" << endl;
        cout << endl;
    }
    cout << " " << endl;
}
